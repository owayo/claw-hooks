...
For decisions that require judgment rather than deterministic rules, you can also use [prompt-based hooks](https://code.claude.com/docs/en/hooks-guide.md#prompt-based-hooks) or [agent-based hooks](https://code.claude.com/docs/en/hooks-guide.md#agent-based-hooks) that use a Claude model to evaluate conditions.

For other ways to extend Claude Code, see [skills](https://code.claude.com/en/skills) for giving Claude additional instructions and executable commands, [subagents](https://code.claude.com/en/sub-agents) for running tasks in isolated contexts, and [plugins](https://code.claude.com/en/plugins) for packaging extensions to share across projects.

<Tip>
  This guide covers common use cases and how to get started. For full event schemas, JSON input/output formats, and advanced features like async hooks and MCP tool hooks, see the [Hooks reference](https://code.claude.com/en/hooks).
</Tip>
...
Hooks let you run code at key points in Claude Code's lifecycle: format files after edits, block commands before they execute, send notifications when Claude needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](https://code.claude.com/en/hooks#hook-lifecycle).

Each example includes a ready-to-use configuration block that you add to a [settings file](https://code.claude.com/docs/en/hooks-guide.md#configure-hook-location). The most common patterns:

* [Get notified when Claude needs input](https://code.claude.com/docs/en/hooks-guide.md#get-notified-when-claude-needs-input)
* [Auto-format code after edits](https://code.claude.com/docs/en/hooks-guide.md#auto-format-code-after-edits)
* [Block edits to protected files](https://code.claude.com/docs/en/hooks-guide.md#block-edits-to-protected-files)
* [Re-inject context after compaction](https://code.claude.com/docs/en/hooks-guide.md#re-inject-context-after-compaction)
...
You can replace the `echo` with any command that produces dynamic output, like `git log --oneline -5` to show recent commits. For injecting context on every session start, consider using [CLAUDE.md](https://code.claude.com/en/memory) instead. For environment variables, see [`CLAUDE_ENV_FILE`](https://code.claude.com/en/hooks#persist-environment-variables) in the reference.
...
Your script can parse that JSON and act on any of those fields. `UserPromptSubmit` hooks get the `prompt` text instead, `SessionStart` hooks get the `source` (startup, resume, compact), and so on. See [Common input fields](https://code.claude.com/en/hooks#common-input-fields) in the reference for shared fields, and each event's section for event-specific schemas.
...
Other events use different decision patterns. For example, `PostToolUse` and `Stop` hooks use a top-level `decision: "block"` field, while `PermissionRequest` uses `hookSpecificOutput.decision.behavior`. See the [summary table](https://code.claude.com/en/hooks#decision-control) in the reference for a full breakdown by event.

For `UserPromptSubmit` hooks, use `additionalContext` instead to inject text into Claude's context. Prompt-based hooks (`type: "prompt"`) handle output differently: see [Prompt-based hooks](https://code.claude.com/docs/en/hooks-guide.md#prompt-based-hooks).
...
For full matcher syntax, see the [Hooks reference](https://code.claude.com/en/hooks#configuration).
...
| [Plugin](https://code.claude.com/en/plugins) `hooks/hooks.json` | When plugin is enabled | Yes, bundled with the plugin |
| [Skill](https://code.claude.com/en/skills) or [agent](https://code.claude.com/en/sub-agents) frontmatter | While the skill or agent is active | Yes, defined in the component file |

You can also use the [`/hooks` menu](https://code.claude.com/en/hooks#the-hooks-menu) in Claude Code to add, delete, and view hooks interactively. To disable all hooks at once, use the toggle at the bottom of the `/hooks` menu or set `"disableAllHooks": true` in your settings file.
...
For full configuration options, see [Prompt-based hooks](https://code.claude.com/en/hooks#prompt-based-hooks) in the reference.
...
For full configuration options, see [Agent-based hooks](https://code.claude.com/en/hooks#agent-based-hooks) in the reference.
...
* `PermissionRequest` hooks do not fire in [non-interactive mode](https://code.claude.com/en/headless) (`-p`). Use `PreToolUse` hooks for automated permission decisions.
...
* [Hooks reference](https://code.claude.com/en/hooks): full event schemas, JSON output format, async hooks, and MCP tool hooks
* [Security considerations](https://code.claude.com/en/hooks#security-considerations): review before deploying hooks in shared or production environments
