...
- 判定はすべて stdout JSON + exit code 0 で返す。フェイルクローズドのパースエラーも exit 0 + `{"decision":"deny","reason":"..."}`（Codex と同じ）
