...
- 公式入力スキーマは camelCase でイベント名フィールドを持たない。`toolCall`、Stop 固有フィールド、`stepIdx`、invocation 固有フィールドからイベントを判別する。後方互換の `hook_event_name` / `event` は空白でない文字列だけ受理する。PreToolUse は `stepIdx`、Stop は `executionNum` / `terminationReason` / `fullyIdle` が必須で、`conversationId`, `workspacePaths`, `transcriptPath`, `artifactDirectoryPath` 等も使用
- PostToolUse の判別は `stepIdx` 単独で行う（`toolCall` を持たず `stepIdx` を持つのは PostToolUse だけなので一意に決まる）。公式仕様の `error` は「Optional。ツール呼び出しが失敗した場合の詳細メッセージ。成功時は空」なので、これを必須条件にすると成功時のペイロードでイベント判別に失敗し、ブロック不可の事後フックにフェイルクローズドの deny を返してしまう
...
- `terminationReason` は文字列であることだけを要求し空文字も受理する（非空の保証は仕様に無く、この値は status / agent_message の情報表示にしか使わない）
...
- 判定はすべて stdout JSON + exit code 0 で返す。フェイルクローズドのパースエラーは PreToolUse では `{"decision":"deny","reason":"..."}` を返す。Stop では `{}`（停止許可）を返す — `continue` は「停止させず再投入する」意味なので、壊れたペイロードに返すと Stop 再発火の無限ループになる（Fail-Closed Security 参照）
