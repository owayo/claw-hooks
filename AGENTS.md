...
- Stop 入力の `agent_type`（非空文字列）は teammate（別プロセスの委譲エージェントセッション）の Stop を意味する。メインセッションの Stop には含まれない（2.1.x の実ペイロードで確認済み: teammate は `claude --agent-id ... --agent-type ...` として起動され、`Uf()` 共通フィールドビルダー経由で `agent_type` が入る）。claw-hooks はこれで `StopSessionKind::Primary / Delegated` を判別し、`session_scope` によるフック選別と NanoBuddy 通知の抑止を行う。欠落・空・非文字列は Primary 扱い（フェイルセーフ方向）。インプロセスのサブエージェント（Task tool）は Stop を発火せず `SubagentStop` のみ
...
- 入力スキーマは camelCase（PreToolUse は `stepIdx`、Stop は `executionNum` / `terminationReason` / `fullyIdle` が必須。`toolCall`, `conversationId`, `workspacePaths`, `transcriptPath`, `artifactDirectoryPath` 等も使用）
