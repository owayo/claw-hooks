...
- 🗜️ **出力圧縮** - 繰り返し装飾文字（`.`, `=`, `-`, `─`, `━`, `^`, `·`, `→`）、進捗表示の末尾の `...` に加え、cargo の `Compiling foo v1.0` のような同じ単語で始まる行が連続するノイズの多い進捗ログをトークン効率のために圧縮。スペースを含むディレクトリ配下の絶対パスでも共通プレフィックスを除去。`^` は ruff / clippy / rustc の長い範囲マーカー、`·` と `→` は Biome などの空白可視化マーカーを縮約する用途。
