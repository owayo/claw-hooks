...
コマンドフック（`[[command_hooks]]`）のために別のエントリを登録する必要はありません。判定器を走らせるのは、以下で各エージェントに登録するコマンドブロック用のエントリです。判定器による拒否はどのエージェントでも効きますが、エージェントへの補足が届くのは Claude Code と Codex CLI（`PreToolUse`）だけです。詳しくは [コマンドフックのプロトコル](cli-reference.ja.md#コマンドフックのプロトコル) を参照してください。
